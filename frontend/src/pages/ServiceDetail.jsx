...
import {
  ArrowLeft, CalendarDays, Clock, Star, CheckCircle, AlertCircle,
  CreditCard, Lock, XCircle, ListChecks, Stethoscope, Sparkles,
} from 'lucide-react'
...
const DetailSkeleton = () => (
  <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 animate-pulse">
    ...
    <div className="bg-white rounded-3xl shadow-md overflow-hidden mb-6">
      <div className="bg-teal-100 h-28" />
      <div className="px-6 pb-6 pt-0">
        <div className="flex gap-4 -mt-14 mb-4">
          <div className="w-28 h-28 rounded-2xl bg-teal-200 flex-shrink-0" />
          <div className="flex-1 pt-16 space-y-2">
            <div className="h-6 bg-teal-100 rounded-full w-3/4" />
            <div className="h-4 bg-teal-100 rounded-full w-1/2" />
          </div>
        </div>
        <div className="flex gap-2 flex-wrap">
          {[80, 100, 64, 88].map(w => (
            <div key={w} className="h-7 bg-teal-100 rounded-full" style={{ width: w }} />
          ))}
        </div>
        <div className="h-20 bg-teal-100 rounded-xl mt-4" />
      </div>
    </div>
...
const ServiceDetail = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        // The getServiceById endpoint returns { success, data } or { success, service }
        const svc = json.data || json.service || null
        if (json.success && svc) {
          if (!cancelled) setService(svc)
        } else {
          throw new Error(json.message || 'Service not found')
        }
      } catch (err) {
        if (!cancelled) setError(err.message || 'Unable to load service details')
      } finally {
  ...
  const futureDates = useMemo(() => {
    ...
    return Object.keys(service.slots)
  ...
  const handleBook = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/service-appointments`, {
        ...
        body: JSON.stringify({
          serviceId: id,          // id from useParams() — same UUID used for GET
  ...
  return (
    <div className="min-h-screen font-serif">
      ...
      {/* ── Loading ──────────────────────────────────────────────────────── */}
      {loading && (
        <div className="bg-linear-to-br from-teal-50 via-white to-emerald-50">
          <DetailSkeleton />
        </div>
      )}
      ...
      {/* ── Error ────────────────────────────────────────────────────────── */}
      {!loading && error && (
        <div className="bg-linear-to-br from-teal-50 via-white to-emerald-50 min-h-[70vh] flex flex-col items-center justify-center px-4 py-24 text-center">
          ...
          <Link
            to="/services"
            className="inline-flex items-center gap-2 bg-teal-600 text-white px-6 py-3 rounded-full font-semibold hover:bg-teal-700 transition-colors shadow-md"
          >
      ...
      {!loading && !error && service && (
        <>
          <div className="bg-linear-to-br from-teal-50 via-white to-emerald-50 min-h-screen pb-2">

            {/* Back link */}
            <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6 pb-2">
              <Link
                to="/services"
                className="inline-flex items-center gap-1.5 text-sm text-teal-600 hover:text-teal-800 font-medium transition-colors group"
              >
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-2 pb-6">
              <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-teal-100">
                {/* Banner */}
                <div className="bg-linear-to-br from-teal-500 to-emerald-600 h-28 relative overflow-hidden">
                ...
                <div className="px-5 sm:px-8 pt-5 sm:pt-6 pb-7">
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-end gap-4 mb-5">
                    <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-2xl border-4 border-white shadow-lg overflow-hidden bg-teal-100 flex-shrink-0">
                      <img
                        src={service.imageUrl || `https://i.pravatar.cc/300?u=service-${id}`}
                        alt={service.name}
                        className="w-full h-full object-cover"
                        onError={e => { e.currentTarget.src = `https://i.pravatar.cc/300?u=service-${id}` }}
                      />
                    </div>
                    <div className="flex-1 sm:pb-2">
                      ...
                      {service.shortDescription && (
                        <p className="text-teal-600 font-medium text-base">{service.shortDescription}</p>
                      )}
                      <div className="mt-2">
                  ...
                  {service.about && (
                    <div className="border-t border-gray-100 pt-5 mt-1">
                      <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">About this Service</h3>
                      <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{service.about}</p>
                    </div>
                  )}
                  ...
                  {service.instructions && service.instructions.length > 0 && (
                    <div className="border-t border-gray-100 pt-5 mt-5">
                      ...
                      <ul className="space-y-2">
                        {service.instructions.map((item, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                          ...
                          <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-medium transition-all duration-200 ${
                              selectedDate === date
                                ...
                                : 'bg-teal-50 text-gray-700 border-teal-100 hover:border-teal-300 hover:bg-teal-100'
                            }`}
                          >
                            <span className="flex justify-between items-center">
                              <span>{displayDate(date)}</span>
                              <span className="text-xs opacity-70">{(service.slots[date] || []).length} slots</span>
                            </span>
                          </button>
                        ))}
                      </div>
                ...
                <div className="lg:col-span-3">
                  <div className="bg-white rounded-2xl shadow-sm border border-teal-100 overflow-hidden">
                    {/* Form header */}
                    <div className="bg-linear-to-r from-teal-500 to-emerald-500 px-6 py-5">
                    ...
                    <div className="p-6">
                      ...
                      {isLoaded && !isSignedIn && (
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                          <div className="w-16 h-16 rounded-full bg-teal-50 border-2 border-teal-200 flex items-center justify-center mb-4">
                            <Lock className="w-7 h-7 text-teal-500" />
                          </div>
                          <h3 className="text-gray-800 font-semibold text-lg mb-2">Sign In to Book</h3>
                          <p className="text-gray-500 text-sm max-w-xs mb-6">
                            Please sign in to book the {service.name} service.
                          </p>
                          <button
                            onClick={() => clerk.openSignIn()}
                            className="inline-flex items-center gap-2 bg-teal-600 text-white px-7 py-3 rounded-full font-semibold hover:bg-teal-700 transition-colors shadow-md"
                          >
                      ...
                      {isLoaded && isSignedIn && !isAvailable && (
                        <div className="flex flex-col items-center justify-center py-10 text-center">
                          ...
                          <p className="text-gray-500 text-sm">
                            This service is not accepting bookings right now. Please check back later.
                          </p>
                        </div>
                      )}
                      ...
                      {isLoaded && isSignedIn && isAvailable && !bookingSuccess && (
                        <form onSubmit={handleBook} className="space-y-4">
                          ...
                          {(selectedDate || selectedTime) && (
                            <div className="bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 text-sm">
                              <p className="text-teal-700 font-medium">
                                ...
                                {selectedDate && selectedTime && <span className="mx-2 text-teal-300">|</span>}
                          ...
                          <div className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 flex justify-between items-center">
                            ...
                            <span className="text-xl font-bold text-teal-600">${service.price ?? 0}</span>
                          </div>
                          ...
                          <button
                            type="submit"
                            disabled={booking}
                            className="w-full flex items-center justify-center gap-2 bg-linear-to-r from-teal-500 to-emerald-500 text-white py-3.5 rounded-xl font-semibold text-sm shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0"
                          >
                            {booking ? (
                              <>
                              ...
                              <>
                                ...
                                Proceed to Payment — ${service.price ?? 0}
                              </>
                            ) : (
                              <>
                                <CalendarDays className="w-4 h-4" />
          ...
          <section className={hs.ctaSection}>
            ...
            <div className={hs.ctaContainer}>
              <h2 className={hs.ctaTitle}>Explore More Services</h2>
              <p className={hs.ctaSubtitle}>
                Discover our full range of medical and wellness services tailored for you.
              </p>
              <div className={hs.ctaButtons}>
                <Link to="/services" className={hs.ctaPrimaryBtn}>
